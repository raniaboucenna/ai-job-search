...
**Self-authored (from Rania's own master CV - her voice, use as the tone reference):**
> Ingénieure en Génie des Procédés avec une expérience en R\&D sur les matériaux biosourcés, la caractérisation physico-chimique et l'analyse de données sous Python. Passionnée par les procédés de valorisation de la biomasse et la modélisation, je souhaite travailler avec vous et contribuer au développement de solutions innovantes pour la transition bas carbone.

*Source: `documents/cv/CV_Rania_Boucenna.pdf` (her own master CV, not a generated draft). Every claim in it is corroborated by `01-candidate-profile.md`. Note the framing worth reusing: the "transition bas carbone" close, and the "Candidate Doctorante : Procédés de Valorisation des Biomasses & Modélisation" headline she chose for herself - "modélisation" is positioning she picked, so keep it visible for thèse applications.*
